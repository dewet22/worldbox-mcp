...
|---|---|---|---|---|---|
| **0.51.2** | 2022.3.60f1 | Mono | **main** (post-0.3.3) | ✅ | HarmonyX reference dropped and Newtonsoft.Json pinned to the game's 13.0.2; 29 tools; verified on macOS (BepInEx 5.4.23.5) incl. `/capabilities`, screenshots, powers, save/load. |
| 0.51.2 | 2022.3.60f1 | Mono | 0.3.0, 0.3.3 | ❌ | Release DLLs (all built with HarmonyX 2.16.1; reproduced with the 0.3.3 ZIP on macOS) reference `MonoMod.Backports`, which BepInEx 5.4.23 does not ship: the plugin fails to load (`FileNotFoundException` in Unity's Player.log, `LogOutput.log` looks normal). Even with a compatible MonoMod present, `/capabilities` throws `MissingMethodException` because Newtonsoft.Json 13.0.4 binds `JToken.ToString(Formatting)`, absent from the game's bundled 13.0.2. |
